...
export default {
    ...
    getSeriesInfo: async (seriesId, type) => {
        let info = {};
        if (seriesId) {
            switch (type) {
                case 'movie':
                    info = await basicFetch(`/movie/${seriesId}?language=pt-BR&api_key=${API_KEY}`);
                    break;
                case 'tv':
                    info = await basicFetch(`/tv/${seriesId}?language=pt-BR&api_key=${API_KEY}`);
                    break;
                default:
                    info = null;
                    break;
